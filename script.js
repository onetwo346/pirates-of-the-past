...
// Boat (Your Perfect Design, Untouched)
...
document.addEventListener('keydown', (event) => {
    switch (event.key) {
        case 'ArrowUp': shipSpeedZ = -0.2; break; // Faster, sharper
...
// Right Joystick: Precise Boat Movement
rightJoystick.on('move', (evt, data) => {
    shipSpeedZ = -data.vector.y * 0.2; // Sharp, smooth
    shipSpeedX = data.vector.x * 0.2;
    shipRotation = -data.vector.x * 0.05; // Tighter turns
});
...
leftJoystick.on('move', (evt, data) => {
    cameraAngleX += data.vector.x * 0.07; // Faster camera
...
function animate() {
    ...
    // Boat Movement (Sharp & Perfect)
    ship.position.x += shipSpeedX;
    ship.position.z += shipSpeedZ;
    ship.rotation.y += shipRotation;
    ship.rotation.z = shipSpeedX * 0.15; // Crisp tilt
    ship.rotation.x = shipSpeedZ * 0.1;  // Sharp pitch
